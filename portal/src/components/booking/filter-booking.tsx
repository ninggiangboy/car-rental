...
const options = [
    ...
    // },
    // {
    //     label: 'Pending confirm deposit',
    //     value: 'PENDING_CONFIRM_DEPOSIT',
    //     field: 'PENDING_CONFIRM_DEPOSIT',
    // },
    // {
...
export default function FilterBooking({ sort }: Readonly<{ sort?: string }>) {
    ...
    const handleSortChange = useDebouncedCallback(() => {
        ...
        if (selected) newParams.set('status', selected.join(','));
        push(createUrl('/booking', newParams));
    }, 600);
            ...
            <Popover>
                <PopoverTrigger asChild>
                    <Button variant="outline">
                        <MixerHorizontalIcon className="mr-2 size-4" /> Status{' '}
                        {selected.length > 0 && (
                            <>
                                <Separator
